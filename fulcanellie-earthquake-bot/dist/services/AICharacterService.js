...
exports.AICharacterService = void 0;
class AICharacterService {
    constructor() {
        this.characterName = "QuakeBot";
        this.personality = {
            name: "QuakeBot",
    ...
    getTimeContext(time) {
        ...
        if (minutes < 5)
            return "Just now - Stay vigilant!";
        if (minutes < 60)
            return `${minutes} minutes ago - Pattern forming...`;
        const hours = Math.floor(minutes / 60);
        if (hours < 24)
            return `${hours} hours ago - Timeline established`;
        return new Date(time).toLocaleString() + " - Historical data point";
    }
    getConspiracyContext(magnitude, place) {
        const lowerPlace = place.toLowerCase();
        let context = "";
        if (magnitude >= 7.0) {
...
exports.AICharacterService = AICharacterService;
