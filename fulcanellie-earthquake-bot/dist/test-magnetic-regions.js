 ...
 */
function displaySolarWindAnalysis(reading) {
 ...
 */
function displayGeographicImpact(noaaService, reading) {
    ...
    const sortedRegions = [...impact.affectedRegions].sort((a, b) => {
        ...
        const getAdjustedScore = (region) => {
            const riskMap = { 'severe': 100, 'high': 75, 'moderate': 50, 'low': 25 };
            const baseScore = riskMap[region.riskLevel] || 0;
...
async function analyzeWeakMagneticRegions() {
    ...
    try {
        ...
        const noaaService = new NOAAService_1.NOAAService();
