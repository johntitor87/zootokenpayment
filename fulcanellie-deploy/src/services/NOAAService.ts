...
/**
 * Interface representing NOAA magnetic field data from solar wind measurements
 */
export interface MagneticFieldReading {
  time_tag: string;
  bx_gsm: number;   // X component of magnetic field in GSM coordinates
  by_gsm: number;   // Y component of magnetic field in GSM coordinates
  bz_gsm: number;   // Z component of magnetic field in GSM coordinates
  lon_gsm: number;  // Longitude in GSM coordinates
  lat_gsm: number;  // Latitude in GSM coordinates
  bt: number;       // Total magnetic field strength in nanoteslas
}

/**
 * Interface representing solar flare data from NOAA
 */
...
/**
 * Solar flare classification and intensity
 */
export interface SolarFlareEvent {
  ...
  impactLevel: 'minimal' | 'minor' | 'moderate' | 'strong' | 'severe';
  affectedRegions: MagneticRegion[];
...
/**
 * Known regions with naturally weaker magnetic fields
 */
export interface MagneticRegion {
  ...
  approximateStrength: number; // in nanoteslas
  riskLevel: 'low' | 'moderate' | 'high' | 'severe';
}

/**
 * Geographical impact assessment for magnetic field events
 */
export interface GeomagneticImpact {
  affectedRegions: MagneticRegion[];
  primaryRegion: MagneticRegion;
  globalImpact: 'minimal' | 'moderate' | 'significant' | 'severe';
  bz_impact: string; // Interpretation of Bz component's effect
  polarEffect: string; // Effect on polar regions
  equatorialEffect: string; // Effect on equatorial regions
}

export interface MagneticFieldAnalysis {
  reading: MagneticFieldReading;
  magneticStrength: number; // in nanoteslas
  status: 'normal' | 'below_normal' | 'weak' | 'superweak';
  timestamp: Date;
  geographicImpact?: GeomagneticImpact; // Optional geographic analysis
}

export class NOAAService {
  private readonly NOAA_URL = 'https://services.swpc.noaa.gov/products/solar-wind/mag-2-hour.json';
  private readonly SOLAR_FLARE_URL = 'https://services.swpc.noaa.gov/json/goes/primary/xrays-6-hour.json';
  private readonly GEOMAGNETIC_URL = 'https://services.swpc.noaa.gov/products/noaa-planetary-k-index.json';
  private readonly KP_INDEX_URL = 'https://services.swpc.noaa.gov/products/noaa-planetary-k-index-forecast.json';
  
  // Adjusted thresholds for solar wind measurements (in nanoteslas)
  private readonly NORMAL_MIN = Number(process.env.MAGNETIC_FIELD_NORMAL_MIN) || 5;
  private readonly NORMAL_MAX = Number(process.env.MAGNETIC_FIELD_NORMAL_MAX) || 15;
  private readonly WEAK_THRESHOLD = Number(process.env.MAGNETIC_FIELD_WEAK_THRESHOLD) || 3;
  private readonly SUPERWEAK_THRESHOLD = Number(process.env.MAGNETIC_FIELD_SUPERWEAK_THRESHOLD) || 1;
  
  // Known weak magnetic field regions worldwide
  private readonly knownWeakRegions: MagneticRegion[] = [
    {
  ...
  private readonly MONITORING_INTERVAL = Number(process.env.NOAA_MONITORING_INTERVAL) || 30 * 60 * 1000; // Default: 30 minutes
   ...
   */
  constructor(twitterService?: any) {
   ...
   */
  async fetchNOAAMagneticData(): Promise<MagneticFieldReading[]> {
    try {
      console.log(`🔍 Fetching magnetic field data from NOAA Solar Wind...`);
      const response = await axios.get(this.NOAA_URL);
      ...
      const magneticReadings = dataRows.map((row: string[]) => {
        ...
        return reading as MagneticFieldReading;
      }).filter(reading => reading !== null);
      
      console.log(`📊 Found ${magneticReadings.length} magnetic field readings`);
      return magneticReadings as MagneticFieldReading[];
    } catch (error) {
   ...
   */
  analyzeMagneticFieldReadings(readings: MagneticFieldReading[]): MagneticFieldAnalysis[] {
    return readings.map(reading => {
      ...
      let status: 'normal' | 'below_normal' | 'weak' | 'superweak' = 'normal';
   ...
   */
  assessGeographicImpact(reading: MagneticFieldReading): GeomagneticImpact {
    ...
    const globalImpact: 'minimal' | 'moderate' | 'significant' | 'severe' = 
      (bz_gsm < -5 && bt > 10) ? 'severe' :
    ...
    if (bz_gsm < -3) {
      ...
      primaryRegion = affectedRegions.find(r => 
        r.name.includes('Polar') || Math.abs(r.centralLatitude) > 60
      ) || primaryRegion;
    } else if (bt < this.WEAK_THRESHOLD) {
      // During weak total field, SAA is most affected
      primaryRegion = affectedRegions.find(r => 
        r.name.includes('Atlantic Anomaly')
      ) || primaryRegion;
    }
   ...
   */
  interpretBzComponent(bz_gsm: number): string {
    if (bz_gsm < -5) {
   ...
   */
  interpretPolarEffect(bz_gsm: number, bt: number): string {
    if (bz_gsm < -3 && bt > 10) {
   ...
   */
  interpretEquatorialEffect(bt: number): string {
   ...
   */
  async fetchSolarFlareData(): Promise<SolarFlareData[]> {
    try {
      console.log('☀️ Fetching solar flare data from NOAA...');
      const response = await axios.get(this.SOLAR_FLARE_URL);
      ...
      const solarFlareData = dataRows.map((row: string[]) => {
        const reading: any = {};
        ...
        return reading as SolarFlareData;
      }).filter(reading => reading !== null);
      
      console.log(`☀️ Found ${solarFlareData.length} solar flare readings`);
      return solarFlareData as SolarFlareData[];
    } catch (error) {
   ...
   */
  analyzeSolarFlareEvents(flareData: SolarFlareData[]): SolarFlareEvent[] {
    const events: SolarFlareEvent[] = [];
   ...
   */
  private groupFlareDataByEvents(flareData: SolarFlareData[]): SolarFlareData[][] {
    // Simple grouping logic - group readings within 6 hours as same event
    const groups: SolarFlareData[][] = [];
    let currentGroup: SolarFlareData[] = [];
   ...
   */
  private classifySolarFlare(peakFlux: number): 'A' | 'B' | 'C' | 'M' | 'X' {
    if (peakFlux >= 1e-3) return 'X'; // X-class
    if (peakFlux >= 1e-4) return 'M'; // M-class
    if (peakFlux >= 1e-5) return 'C'; // C-class
    if (peakFlux >= 1e-6) return 'B'; // B-class
    return 'A'; // A-class
  }
   ...
   */
  private calculateFlareIntensity(peakFlux: number): number {
   ...
   */
  private calculateFlareDuration(eventData: SolarFlareData[]): number {
    if (eventData.length <= 1) return 0;
   ...
   */
  private assessFlareImpact(classification: string, peakFlux: number): 'minimal' | 'minor' | 'moderate' | 'strong' | 'severe' {
   ...
   */
  private identifyAffectedRegions(classification: string, peakFlux: number): MagneticRegion[] {
    let affectedRegions = [...this.knownWeakRegions];
   ...
   */
  private predictEarthquakeFromFlare(classification: string, peakFlux: number): {
    likelyRegions: string[];
    timeframe: string;
    confidence: number;
  } | undefined {
   ...
   */
  formatSolarFlareEarthquakePrediction(flareEvents: SolarFlareEvent[]): string {
   ...
   */
  formatMagneticFieldAlert(analysis: MagneticFieldAnalysis): string {
    const { reading, magneticStrength, status, timestamp } = analysis;
    // Status-specific emojis and descriptions
    const statusInfo: Record<string, { emoji: string; description: string; mystical: string }> = {
      normal: {
        emoji: '✅',
